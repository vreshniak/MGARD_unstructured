...
int main(int argc, char *argv[])
{
	...
	for (auto &block : blocks){
		...
		// sort variables
		for (int i=0; i<var_names.size(); i++) std::sort(vars[i].begin(), vars[i].end());
		...
		for (int i=0; i<var_names.size(); i++){
			...
			// mag_v = 0;
			// for (size_t k=0; k<N; k++){
			// 	mag_v += (*((DTYPE*)decompressed.data()+k)-vars[i][k]) * ( *((DTYPE*)decompressed.data()+k)-vars[i][k]) / N;
			// }
			// double L2_norm_err = std::sqrt(mag_v);

			avg_err += L2_err / (L2_norm+1.e-5) / vars.size();
		}
