...
int main(int argc, char *argv[])
{
	...
	while (true){
		...
		// Build my block indices (round-robin split)
		...
		// Per-rank, per-step output buffers (concatenate all my blocks)
		std::vector<int64_t> out_ElementConnectivity;
		std::vector<std::vector<double>> out_merged_coos(coo_names.size());
		std::vector<std::vector<double>> out_merged_vars(var_names.size());

		for (size_t bi : myBlockIdx){
			const auto &block = blocks[bi];

			auto start = std::chrono::high_resolution_clock::now();
			if (verbose && rank == 0) std::cout << "step = " << step << ", block# " << block.BlockID+1 << " out of " << nblocks << std::endl;
			...
			// Append to per-rank buffers
			out_ElementConnectivity.insert(out_ElementConnectivity.end(), ElementConnectivity.begin(), ElementConnectivity.end());

			for (size_t i = 0; i < coo_names.size(); ++i)
				out_merged_coos[i].insert(out_merged_coos[i].end(), merged_coos[i].begin(), merged_coos[i].end());

			for (size_t i = 0; i < var_names.size(); ++i)
				out_merged_vars[i].insert(out_merged_vars[i].end(), merged_vars[i].begin(), merged_vars[i].end());


			//////////////////////////////////////////////////////////////////////////////////
			// save merged variables

			// // One write per variable per step per rank

			// // Connectivity
			// out_adios_connectivity.SetSelection(adios2::Box<adios2::Dims>({}, {out_ElementConnectivity.size()}));
			// if (!out_ElementConnectivity.empty())
			// 	bpWriter.Put<int64_t>(out_adios_connectivity, out_ElementConnectivity.data(), adios2::Mode::Sync);

			// // Coordinates
			// for (size_t i = 0; i < coo_names.size(); ++i)
			// {
			// 	out_adios_coos[i].SetSelection(adios2::Box<adios2::Dims>({}, {out_merged_coos[i].size()}));
			// 	if (!out_merged_coos[i].empty())
			// 		bpWriter.Put<double>(out_adios_coos[i], out_merged_coos[i].data(), adios2::Mode::Sync);
			// }

			// // Variables
			// for (size_t i = 0; i < var_names.size(); ++i)
			// {
			// 	out_adios_vars[i].SetSelection(adios2::Box<adios2::Dims>({}, {out_merged_vars[i].size()}));
			// 	if (!out_merged_vars[i].empty())
			// 		bpWriter.Put<double>(out_adios_vars[i], out_merged_vars[i].data(), adios2::Mode::Sync);
			// }
